...
--
-- Host: localhost    Database: coffeezin
-- ------------------------------------------------------
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_client`(email_input varchar(50))
    MODIFIES SQL DATA
begin
...
begin
	select id, convert(email using utf8) as email, convert(created_at using utf8) created_at 
	from client
    order by created_at;
END ;;
DELIMITER ;
